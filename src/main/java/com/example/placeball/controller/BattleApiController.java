...
public class BattleApiController {
    ...
    public ResponseEntity<List<Map<String, Object>>> getTodayBattle(
        ...
        // 오늘 (myTeam) 경기가 없으면 → myTeam의 가장 가까운 경기로 폴백 (점수판이 빈 채로 멈추지 않도록)
        if (games.isEmpty() && team != null && !team.isBlank()) {
            var pageable = org.springframework.data.domain.PageRequest.of(0, 1);
            List<Game> near = gameRepository.findUpcomingByTeam(team, today, pageable);
            if (near.isEmpty()) near = gameRepository.findPastByTeam(team, today, pageable);
            games = near;
        }
    ...
    public ResponseEntity<Map<String, Object>> getZoneBattle(
        ...
        // 오늘 경기가 없으면 → myTeam의 가장 가까운 경기(예정 우선, 없으면 최근)로 폴백
        if (game == null && team != null && !team.isBlank()) {
            var pageable = org.springframework.data.domain.PageRequest.of(0, 1);
            List<Game> upcoming = gameRepository.findUpcomingByTeam(team, today, pageable);
            if (!upcoming.isEmpty()) {
                game = upcoming.get(0);
            } else {
                List<Game> past = gameRepository.findPastByTeam(team, today, pageable);
                if (!past.isEmpty()) game = past.get(0);
            }
        }

        // 그래도 경기를 못 찾으면 → myTeam을 홈으로 한 가상 매치업 (점수가 사라지지 않게)
        boolean virtualGame = false;
        ...
        for (String zone : zones) {
            ...
            int total = home + away;
            // 활동(포인트)이 전혀 없으면 양 팀 모두 0% — 합산이 항상 0 또는 100이 되도록 보장
            int homePct = total == 0 ? 0 : Math.round((float) home / total * 100);
            int awayPct = total == 0 ? 0 : 100 - homePct;
        ...
        // 오늘 온라인 점수가 0이면 → 최근 7일 누적으로 폴백 (시간대/날짜 경계로 점수가 안 보이는 문제 방지)
        if (onlineHome == 0 && onlineAway == 0) {
            LocalDateTime weekFrom = today.minusDays(6).atStartOfDay();
            onlineHome = cheerPointRepository.sumOnlineByTeamAndPeriod(homeTeam, weekFrom, to);
            onlineAway = cheerPointRepository.sumOnlineByTeamAndPeriod(awayTeam, weekFrom, to);
        }

        int onlineTotal = onlineHome + onlineAway;
        // 온라인 활동(출석/글/댓글 등)이 전혀 없으면 양 팀 모두 0%
        int onlineHomePct = onlineTotal == 0 ? 0 : Math.round((float) onlineHome / onlineTotal * 100);
        int onlineAwayPct = onlineTotal == 0 ? 0 : 100 - onlineHomePct;

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("gameId",   game != null ? game.getId() : 0);
        result.put("virtual",  virtualGame);
        result.put("homeTeam", homeTeam);
        result.put("awayTeam", awayTeam);
        result.put("zones",    zoneResult);
        result.put("online",   Map.of(
                "homeTeam", homeTeam, "awayTeam", awayTeam,
                "homeScore", onlineHome, "awayScore", onlineAway,
                "homePct", onlineHomePct, "awayPct", onlineAwayPct));
        return ResponseEntity.ok(result);
    }
    ...
    private Map<String, Object> buildBattleStatus(Game game, LocalDate date) {
        ...
        // 오늘 온라인 점수가 0이면 → 최근 7일 누적으로 폴백 (날짜 경계로 점수가 안 보이는 문제 방지)
        if (homeOnline == 0 && awayOnline == 0) {
            LocalDateTime weekFrom = date.minusDays(6).atStartOfDay();
            homeOnline = cheerPointRepository.sumOnlineByTeamAndPeriod(game.getHomeTeam(), weekFrom, to);
            awayOnline = cheerPointRepository.sumOnlineByTeamAndPeriod(game.getAwayTeam(), weekFrom, to);
        }

        int homeTotal   = homeTicket + homeOnline;
        int awayTotal   = awayTicket + awayOnline;
        int total       = homeTotal  + awayTotal;
        // 양 팀 활동이 전혀 없으면 모두 0% (합산이 0 또는 100이 되도록 보장)
        int homePct     = total == 0 ? 0 : Math.round((float) homeTotal / total * 100);
        int awayPct     = total == 0 ? 0 : 100 - homePct;
